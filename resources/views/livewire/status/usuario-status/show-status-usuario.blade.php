<div>
    ...
    <div class="ml-auto mb-6 lg:w-[75%] xl:w-[80] 2xl:w-[85%]">
        <div class="container mx-auto bg-gray-800  rounded-lg">
            <h2 class="flex justify-center mt-5 py-3 font-bold text-white">Módulo de Gestion de Estado del Usuario</h2>
        </div>
                ...
                <div class="mt-5 flex  bg-gray-100 font-sans overflow-hidden rounded-lg">
                    <div class="w-full">
                        ...
                        <div class="flex justify-around bg-gray-200 items-center  py-3">
                            ...
                            <!--boton de crear-->
                            <div>
                                <a href="{{ route('crear-status-usuario') }}">
                                    <button class="btn-agregar ">Crear Nueva Estado</button>
                                </a>
                            </div>
        ...
        <div class="container mx-auto">
            <div class="bg-white shadow-md rounded my-6 ">
                @if ($estado_usuarios->count())
                    <table class="min-w-max w-full table-auto">
                        <thead>
                            <tr class="bg-gray-200 text-gray-600 uppercase text-sm ">

                                <th class="py-3 px-6 text-left">ID</th>
                                <th class="py-3 px-6 text-left">Nombre Estado</th>
                                <th class="py-3 px-6 text-center">Acciones</th>
                            </tr>
                        </thead>

                        <tbody class="text-gray-600 text-sm font-light">
                            @foreach ($estado_usuarios as $estado_usuario)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">


                                    <td class="py-1 px-6 text-center">
                                        <div class="flex items-center font-bold uppercase">
                                            <span>{{ $estado_usuario->id }}</span>
                                        </div>
                                    </td>

                                    <td class="py-1 px-6 text-center">
                                        <div class="flex items-center font-bold uppercase ">
                                            <span>{{ $estado_usuario->nombre }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-1 px-6 text-center">
                                        <div class="flex item-center justify-center">
                                            ...
                                            <div class="flex items-center justify-around py-[4px]">
                                                <a
                                                    href="{{ route('editar-status-usuario', ['id' => $estado_usuario->id]) }}">
                                                    <button class="btn-editar mb-1">Editar</button>
                                                </a>
                                            </div>

                                            <div class="mt-1 px-3">
                                                <a onclick="confirm('¿Estas Seguro de Eliminar El Estado del Usuario?')||event.stopImmediatePropagation()"
                                                    wire:click="destroy({{ $estado_usuario->id }})"><button
                ...
                <div class="bg-gray-200">
                    @if ($estado_usuarios->hasPages())
                        <div class="px-6 py-3 ">
                            {{ $estado_usuarios->links() }}
                        </div>
                    @endif
                </div>
            </div>
