<div>
    ...
    <div class="ml-auto mb-6 lg:w-[75%] xl:w-[80] 2xl:w-[85%]">
        <div class="container mx-auto bg-gray-800  rounded-lg">
            <h2 class="flex justify-center mt-5 py-3 font-bold text-white">Módulo de Gestion de Estado Ticket</h2>
        </div>
        ...
        <div class="container mx-auto">
            <div class="bg-white shadow-md rounded my-6 ">
                @if ($estado_tickets->count())
                    <table class="min-w-max w-full table-auto">
                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                            @foreach ($estado_tickets as $estado_ticket)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">


                                    <td class="py-1 px-6 text-center">
                                        <div class="flex items-center font-bold uppercase">
                                            <span>{{ $estado_ticket->id }}</span>
                                        </div>
                                    </td>

                                    <td class="py-1 px-6 text-center">
                                        <div class="flex items-center font-bold uppercase ">
                                            <span>{{ $estado_ticket->nombre }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-1 px-6 text-center">
                                        <div class="flex item-center justify-center">
                                            ...
                                            <div class="flex items-center justify-around py-[4px]">
                                                <a
                                                    href="{{ route('editar-ticket', ['id' => $estado_ticket->id]) }}">
                                                    <button class="btn-editar mb-1">Editar</button>
                                                </a>
                                            </div>

                                            <div class="mt-1 px-3">
                                                <a onclick="confirm('¿Estas Seguro de Eliminar El Estado del Ticket?')||event.stopImmediatePropagation()"
                                                    wire:click="destroy({{ $estado_ticket->id }})"><button
                ...
                <div class="bg-gray-200">
                    @if ($estado_tickets->hasPages())
                        <div class="px-6 py-3 ">
                            {{ $estado_tickets->links() }}
                        </div>
                    @endif
                </div>
            </div>
