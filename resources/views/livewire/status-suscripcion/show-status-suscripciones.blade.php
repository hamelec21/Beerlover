        ...
        <div class="container mx-auto">
            <div class="bg-white shadow-md rounded my-6 ">
                @if ($status_suscripciones->count())
                    <table class="min-w-max w-full table-auto">
                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                            @foreach ($status_suscripciones as $status_suscripcion)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">


                                    <td class="py-1 px-6 text-center">
                                        <div class="flex items-center font-bold uppercase">
                                            <span>{{ $status_suscripcion->id }}</span>
                                        </div>
                                    </td>

                                    <td class="py-1 px-6 text-center">
                                        <div class="flex items-center font-bold uppercase ">
                                            <span>{{ $status_suscripcion->nombre }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-1 px-6 text-center">
                                        <div class="flex item-center justify-center">
                                            ...
                                            <div class="flex items-center justify-around py-[4px]">
                                                <a
                                                    href="{{ route('editar-status-suscripciones', ['id' => $status_suscripcion->id]) }}">
                                            ...
                                            <div class="mt-1 px-3">
                                                <a onclick="confirm('¿Estas Seguro de Eliminar El Tipo de Suscripcion?')||event.stopImmediatePropagation()"
                                                    wire:click="destroy({{ $status_suscripcion->id }})"><button
                ...
                <div class="bg-gray-200">
                    @if ($status_suscripciones->hasPages())
                        <div class="px-6 py-3 ">
                            {{ $status_suscripciones->links() }}
                        </div>
                    @endif
                </div>
            </div>
